...
Rimpact has been tested to work with reference data exported from Scopus in the RIS format. For most other bibliographic databases, such as PubMed, the OvidSP databases, and the Web of Knowledge databases, it is recommended that you first export data to a citation management application, such as EndNote or RefWorks. You can then use these applications to clean up (if necessary) and export the data to the RIS format.
...
7. Click "Export", and the data file will be downloaded to your computer (default file name is "scopus.ris").
8. You can copy the downloaded data file to your application's +public/data+ directory. Rename it if you wish. Typically, you would end up having a file like this:
...
2. Clean up references in EndNote # See section below on "Cleaning reference data".
...
6. In Output Style drop-down, choose "RefMan (RIS) Export".
7. Click "Save".
...
2. Clean up references in RefWorks. # See section below on "Cleaning reference data".
3. Select the references you wish to export to RIS format in RefWorks.
4. Go to "References", "Export".
5. At "Select an export format", choose "Bibliographic Software".
6. Click "Export References".
7. Move the exported file to your application's "public/data" directory.

== Cleaning Reference Data

Your visualization can only be as good as the integrity of the reference data it is based on. The reference data you download from online databases is unfortunately very likely to contain errors and inconsistencies. You should use applications such as EndNote, RefWorks, or any editing tool you are comfortable with, to clean up the data. Here are some common problems to pay attention to.
...
Are there authors that have more than one names in your data? For example:
  ...
  Grosetta, Holly # Maiden name
  Nardini, Holly Grosetta # Name changed after marriage
...
Rimpact works best if each value is in its own line in a multi-valued field. However, some databases string values together and use some sort of delimiter (; or  |) to separate them. For example:
  Smith, JD; Jone, AD; ...
...
== Visualizing Reference Data Using Recipes

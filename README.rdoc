...
Rimpact has been tested to work with reference data exported from Scopus in the RIS format. For most other bibliographic databases, such as PubMed, the OvidSP databases, the Web of Knowledge databases, or if you are using data retrieved from more than one sources, it is recommended that you first export data to a citation management application, such as EndNote or RefWorks. You can then use these applications to clean up (if necessary) and then export the data to the RIS format.

...
Are there addresses, especially those that only has the city name, that refer to more than places in the world? For example:
...
Are there inconsistent spellings for place names? For example:
  CT, ct, Conn., Connecticut
  USA, U.S.A., United States, United States of America, US, U.S.
	
=== Multiple field values

Rimpact works best if each value is in its own line in the field. However, some databases string them together and use some sort of delimiter (; or  |) to separate them. For example:
  Smith, JD; Jone, AD; ...
  Yale University, New Haven, Connecticut, United States; Stanford University, Stanford, California, United States; ...

In that case, you should manually change the record (using a program such as EndNote) so that each value is in its own line, or change your code so that it will accomodate these special delimiter characters.

=== Duplicate references

If you are getting data from different sources, you are likely to get duplicates in your data set. You should try your best to remove the duplicates so that your visualization is as accurate as it can be. Citation management programs such as EndNote and RefWorks have "Find Duplicates" features, but please note that due to subtle differences in the references data from different sources, these programs may not be able to find all duplicates.

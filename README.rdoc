...
Your visualization is only as good as the integrity of the reference data it is based on. The reference data you download from online databases is unfortunately very likely to contain errors and inconsistencies. You should try your best and use tools such as EndNote, RefWorks, or Microsoft Excel to clean up the data. Here are some common problems with reference data.

=== Ambiguous author names

1. Are there authors that have more than one names in your data? For example:
...
2. Are there more than one authors that have the same name? For example:
	Shepherd, G # The senior
	Shepherd, G # The junior
	
=== Ambiguous place names

1. Are there addresses, especially those that only has the city name, that refer to more than places in the world? For example:
...
2. Are there inconsistent spellings for place names? For example:

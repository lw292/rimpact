...
Rimpact allows your Ruby on Rails application to parse bibliographic data in {BibTeX}[http://en.wikipedia.org/wiki/BibTeX] or {RIS}[http://en.wikipedia.org/wiki/RIS_(file_format)] formats and generate {d3.js}[http://d3js.org/] based visualization graphs.
...
Rimpact has been tested to work with reference data exported from Scopus in the BibTeX or RIS formats. For most other bibliographic databases, such as PubMed, the OvidSP databases, the Web of Knowledge databases, or if you are using data retrieved from more than one databases, it is recommended that you first export data to a citation management application, such as EndNote or RefWorks. You can then use these applications to clean up the data (see the "Cleaning up Reference Data" section below) and then export to the BibTeX or RIS formats.
...
1. Run your search in your favorite databases, and export references to EndNote.
2. Clean up references in EndNote. See the "Cleaning up Reference Data" section below.
3. Select the references you wish to export to RIS format in EndNote.
4. Go to "File", "Export".
5. Navigate to a location where you wish to save the exported file, and choose a file name.
6. In Output Style drop-down, for RIS format, choose "RefMan (RIS) Export". If it is not available in the dropdown, find them using "Select Another Style".
7. Click "Save".
8. Move the exported file to your application's "public/data" directory.

==== EndNote (BibTeX)

TODO: Instructions for exporting references (with all the required fields) from EndNote to BibTeX, and using a BibTeX editor to auto-generate BibTeX keys.

==== RefWorks

1. Run your search in your favorite databases, and export references to RefWorks.
2. Clean up references in RefWorks. See the "Cleaning up Reference Data" section below.
...
Rimpact works best if each value is in its own line in a multi-valued field. However, some databases string values together and use some sort of delimiter (such as ";" or "|") to separate them. For example:
  Smith, JD; Jone, AD; ...
  Yale University, New Haven, Connecticut, United States; Stanford University, Stanford, California, United States; ...

In that case, you should change the record (using a program such as EndNote) so that each value is in its own line, or change your code so that it will accomodate these special delimiter characters.
...
== Transforming and Visualizing Data with "Recipes"

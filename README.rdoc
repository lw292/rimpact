...
Add the following lines to the +Gemfile+:
  gem 'gnlookup', :git => 'https://github.com/lw292/gnlookup.git' # Rimpact's geo recipe depends upon gnlookup. More information below.
  gem 'rimpact', :git => 'https://github.com/lw292/rimpact.git'

Then use the +bundle+ command to install the gems and their dependencies:
  bundle install

Then you will need to create some required files and directories in your application. You can do so by running:
  rake rimpact:setup
...
== Getting RIS data from Scopus

Rimpact has been tested to work with the RIS data exported from Scopus. Data from other sources will be tested in the future. Here is how to get RIS data from Scopus:
...
You can create your own custom recipes following the example of the included recipes. If you would like to contribute your recipes to us, please let us know so that we can add it to Rimpact.
...
This will parse your RIS data file, extract the author fields, calculate the number of collaborators and publications for each author, and present the results in a force-directed graph.

==== Ambiguous author names

You should disambiguate author names before running this task. Rimpact assumes that authors by exactly the same name in your data are the same author and names that are not exactly the same belong to different authors.
...
This will parse your RIS data file, extract the affiliation address fields, and for each address, and if an address can be identified, try to determine the latitude and longitude of the address at city level.

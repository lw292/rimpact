class Address
  ...
  def cities
    ...
    pieces.each do |piece|
      ...
      cities << Gnlookup::City.where("name = ?", value)
    end
  ...
  def regions
    ...
    pieces.each do |piece|
      ...
      if !(us_address_pattern =~ value).nil? # If this matches a US address pattern
        ...
        regions << Gnlookup::Region.where("name = ? OR iso = ?", state_name, state_name) # Search with state names
      else
        regions << Gnlookup::Region.where("name = ? OR iso = ?", value, value) # Search with raw value
      end
    end
  ...
  def countries
    ...
    pieces.each do |piece|
      value = piece.strip.chomp(".")
      countries << Gnlookup::Country.where("name = ? OR iso = ? OR iso3 = ?", value, value, value)
    end
